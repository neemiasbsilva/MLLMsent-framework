...
class Translate: 
    ...
    def augment(self, data):
        ...
        blob = TextBlob(data.lower())
        try:
            new_data = blob.translate(from_lang=self.src, to=self.to)
            new_data = blob.translate(from_lang=self.to, to=self.src)
        except:
            try:  # Switch to googletrans to do translation.
                translator = Translator()
                new_data = translator.translate(data, dest=self.to, src=self.src).text
                new_data = translator.translate(data, dest=self.src, src=self.to).text
            except Exception:
                print("Error Not translated.\n")
                raise

        return str(new_data).lower()
